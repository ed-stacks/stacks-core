...
impl<'a, 'b> Environment<'a, 'b> {
    ...
    pub fn initialize_contract_with_db(
        ...
        .map_err(|boxed_err| {
            let (check_error, _cost_tracker) = *boxed_err;
            let err = *check_error.err;
            error!("Analysis step has failed: {err:?}");
            VmExecutionError::Unchecked(CheckErrorKind::Expects(format!("{err:?}")))
        })?;
...
impl ContractContext {
    ...
    pub fn with_wasm_module<T>(&self, f: impl Fn(&[u8]) -> Result<T>) -> Result<T> {
        let wasm_module =
            self.wasm_module
                .as_ref()
                .ok_or(crate::vm::errors::VmExecutionError::Wasm(
...
mod test {
    ...
    fn vm_initialize_contract_already_exists(
        ...
        env.initialize_contract(contract_id.clone(), contract_src)
            .unwrap();

        // Second initialization hits ContractAlreadyExists
        let err = env
            .initialize_contract(contract_id.clone(), contract_src)
            .unwrap_err();
