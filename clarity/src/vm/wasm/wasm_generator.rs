...
use crate::vm::types::signatures::{CallableSubtype, StringUTF8Length};
use crate::vm::types::{
...
use crate::vm::wasm::cost::{ChargeContext, WordCharge};
use crate::vm::wasm::error_mapping::ErrorMap;
...
use crate::vm::wasm::{debug_msg, words};
use crate::vm::{functions, variables, ClarityName, SymbolicExpression, SymbolicExpressionType};
...
impl WasmGenerator {
    ...
    pub fn type_for_serialization(&self, ty: &TypeSignature) -> TypeSignature {
        use crate::vm::types::signatures::TypeSignature::*;
        match ty {
    ...
    pub(crate) fn add_literal(&mut self, value: &Value) -> Result<(u32, u32), GeneratorError> {
        let data = match value {
            ...
            Value::Sequence(SequenceData::Buffer(buff_data)) => buff_data.data.clone(),
            Value::Sequence(SequenceData::String(string_data)) => {
    ...
    fn visit_literal_value(
        ...
        match value {
            ...
            | Value::Response(_)
            | Value::CallableContract(_)
            | Value::Sequence(_) => Err(GeneratorError::TypeError(format!(
                "Not a valid literal type: {value:?}"
            ))),
        }
    }
...
",
    ...
    fn end_of_standard_data_is_correct() {
        const STANDARD_LIB_PATH: &str = concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/src/vm/wasm/standard/standard.wasm"
        );
    ...
    fn function_argument_have_correct_type() {
        ...
        assert!(crate::vm::wasm::tools::evaluate_at(
            snippet,
