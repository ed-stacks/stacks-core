...
use crate::vm::types::{FixedFunction, FunctionType, TypeSignature};
use crate::vm::wasm::cost::WordCharge;
use crate::vm::wasm::error_mapping::ErrorMap;
use crate::vm::wasm::wasm_generator::{
    add_placeholder_for_clarity_type, drop_value, ArgumentsExt, GeneratorError,
    SequenceElementType, WasmGenerator,
};
use crate::vm::wasm::wasm_utils::{check_argument_count, ArgumentCountCheck};
use crate::vm::wasm::words;
...
#[cfg(test)]
mod tests {
    use clarity_types::types::OptionalData;

    use crate::vm::errors::{EarlyReturnError, VmExecutionError as Error};
  ...
  (some (+ (try! x) 10)))";
    ...
    fn try_optional_false() {
        crosscheck(
            ...
            Err(Error::EarlyReturn(EarlyReturnError::UnwrapFailed(
                Box::new(Value::Optional(OptionalData { data: None })),
            ))),
        )
    }
