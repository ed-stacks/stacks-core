...
use crate::vm::types::{ASCIIData, CharType};
use crate::vm::wasm::cost::WordCharge;
use crate::vm::wasm::wasm_generator::{ArgumentsExt, GeneratorError, WasmGenerator};
use crate::vm::wasm::wasm_utils::{
    check_argument_count, signature_from_string, ArgumentCountCheck,
};
...
mod tests {
    ...
    use crate::vm::types::{ASCIIData, CharType, ListTypeData, SequenceData, TupleData, UTF8Data};
...
(define-public (foo (a int))
  (ok (print a))
        ...
        crate::vm::wasm::tools::crosscheck_multi_contract(
            &[
        ...
        crosscheck(
            ...
            Ok(Some(
                Value::list_with_type(
                    ...
                    vec![],
                    ListTypeData::new_list(TypeSignature::NoType, 0).unwrap(),
                )
                .unwrap(),
            )),
        );
