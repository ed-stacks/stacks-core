...
pub enum ClarityError {
    ...
    Parse(ParseError),
    /// Error originating from the WebAssembly runtime
    #[cfg(feature = "clarity-wasm")]
    Wasm(WasmError),
...
impl fmt::Display for ClarityError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            #[cfg(feature = "clarity-wasm")]
            ClarityError::Wasm(ref e) => fmt::Display::fmt(e, f),
        }
    }
}
...
impl std::error::Error for ClarityError {
    fn cause(&self) -> Option<&dyn std::error::Error> {
        match *self {
            ...
            ClarityError::BadTransaction(_) => None,
            ClarityError::CostError(_, _) => None,
