...
impl StandardPrincipalData {
    pub fn new(version: u8, bytes: [u8; 20]) -> std::result::Result<Self, VmInternalError> {
        if version >= 32 {
...
impl Value {
    ...
    pub fn string_utf8_from_unicode_scalars(scalars: Vec<u8>) -> Result<Value, VmExecutionError> {
        // Ensure input length is divisible by 4
        if scalars.len() % 4 != 0 {
            return Err(CheckErrorKind::InvalidCharactersDetected.into());
        }

        let chars_result: Result<Vec<char>, VmExecutionError> = scalars
            .chunks_exact(4)
            .map(|chunk| {
                let value = u32::from_be_bytes([chunk[0], chunk[1], chunk[2], chunk[3]]);
                std::char::from_u32(value).ok_or(CheckErrorKind::InvalidCharactersDetected.into())
            })
            .collect();
